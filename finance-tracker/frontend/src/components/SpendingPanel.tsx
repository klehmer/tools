...
import type { SpendingBreakdown, SpendingBucket, SpendingCategory } from "../types";
...
const BUCKET_META: {
  ...
  { key: "subscriptions", title: "Subscriptions", color: "border-indigo-100", headerBg: "hover:bg-indigo-50" },
...
function MiniCard({ label, total, count }: { label: string; total: number; count: number }) {
  return (
          ...
          <tbody>
            {bucket.transactions.map((t, i) => (
              <tr key={i} className="border-t border-slate-50">
                ...
                <td className="px-5 py-2 text-slate-700 truncate max-w-xs">{t.name}</td>
