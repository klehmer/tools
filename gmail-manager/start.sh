#!/usr/bin/env bash
# Start the Gmail Manager app (backend + frontend).
...
BACKEND_LOG="/tmp/gmail-backend.out"
FRONTEND_LOG="/tmp/gmail-frontend.out"

stop() {
  echo "Stopping..."
  pkill -f "uvicorn main:app" 2>/dev/null || true
  pkill -f "vite.*gmail-manager" 2>/dev/null || true
